...
public extension Date {
    ...
    func dateStringFull() -> String {
        let dateFormatter = DateFormatter()
        dateFormatter.timeZone = TimeZone(identifier: "es_EC")!
    ...
    /// Week of month.                  //calendar bug fixed
    ...
    static func fromUTCToLocalDate(stringDate: String, _format:String=kDateFormatZZZZ) -> Date {
        ...
        let localDate = Date.convertDateToLocalDate(date: date)
        
        return localDate        
    }
    
    static func convertDateToLocalDate(date: Date) -> Date {
        let localDateFormatter:DateFormatter = DateFormatter()
        localDateFormatter.timeStyle = DateFormatter.Style.full
        localDateFormatter.dateStyle = DateFormatter.Style.full
        localDateFormatter.timeZone = TimeZone.current
        let localDateString = localDateFormatter.string(from: date)
        
        let localDate = localDateFormatter.date(from: localDateString)
        return localDate!
    }
    
    static func convertDateToLocalString(date: Date) -> String {
        let localDateFormatter:DateFormatter = DateFormatter()
        localDateFormatter.timeStyle = DateFormatter.Style.long
        localDateFormatter.dateStyle = DateFormatter.Style.long
        localDateFormatter.timeZone = TimeZone.current
        let localDateString = localDateFormatter.string(from: date)
    ...
    static func currentLocalMonth() -> String {
        let today = Date()
        let todayString = today.toStringWithFormat(format: "yyyy-MM-dd:HH:mm:ssZ")
        let components = todayString.components(separatedBy: "-")
        let monthString = components[1]
        return monthString
    }
    
    static func dateIsInTheFuture(date: Date) -> Bool {
        let dateString = Date.convertDateToLocalString(date: date)
        let dateString2 = Date.convertDateToLocalString(date: Date())
    ...
    static func toDateWithFormat(dateString: String, format: String) -> Date {
        let dateFormatter = DateFormatter()
...
//        dateFormatter.timeZone = TimeZone(identifier: "UTC")!
    ...
    static func toGMTDateWithFormat(dateString: String, format: String) -> Date {
        let dateFormatter = DateFormatter()
        dateFormatter.dateFormat = format //Your date format
        let date = dateFormatter.date(from: dateString) //according to date format your date string
    ...
    func toStringWithLongFormat() -> String {
        let longDate = toStringWithFormat(format: "EEEE, dd").capitalized + " de " + toStringWithFormat(format: "MMMM").capitalized + " de " + toStringWithFormat(format: "YYYY")
        return longDate
    }
    
    func startOfMonth() -> Date? {
        let comp: DateComponents = Date.getCalendar().dateComponents([.year, .month, .hour], from: Date.getCalendar().startOfDay(for: self))
        return Date.getCalendar().date(from: comp)!
    }
    
    func endOfMonth() -> Date? {
        let d = Date.init()
        var comp: DateComponents = d.calendar.dateComponents([.month, .day, .hour], from: Date.getCalendar().startOfDay(for: self))
        comp.month = 1
        comp.day = -1
    ...
    func dayOfWeek() -> Int {
        let dateComponents = Date.getCalendar().dateComponents([.weekday], from: self)
    ...
    func absoluteWeekOfMonth() -> Int {
        let dateComponents = Date.getCalendar().dateComponents([.weekOfMonth], from: self)
    ...
    func getWeekOfMonth() -> Int {
        ...
        var week = dateComponents.weekOfMonth!
        
        let start_date = startOfMonth()
        
        let absolute_start_week = start_date!.absoluteWeekOfMonth()
        
        if absolute_start_week == 0 {
            week += 1
        }
